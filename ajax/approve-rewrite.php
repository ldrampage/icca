<?php
    ...
    if(isset($_POST['id']) && isset($_POST['reviewer']) && isset($_POST['old_status'])) {
        ...
        if(intval($rewrite['is_open']) || (!intval($rewrite['is_open']) && $diffSeconds >= 60)) {
            # update status
            if($_POST['reviewer']) {
                $result = $icca_new_obj->updateStatus($_POST['id'], 1, $_POST['reviewer'], $_POST['old_status']);
            } else {
                $result = $icca_new_obj->updateStatus($_POST['id'], 1, $_SESSION['login_id'], $_POST['old_status']);
            }
